...
@if (!empty($field['value']))
    <div class="existing-file">
        <a target="_blank"
           @php
               $url = explode('/', $field['value']);
               $file = \Different\Dwfw\app\Models\File::find(array_pop($url));
           @endphp
           @if (isset($field['disk']))
           ...
           href="{{ (asset(\Storage::disk($field['disk'])->temporaryUrl(Arr::get($field, 'prefix', '').$field['value'], Carbon\Carbon::now()->addMinutes($field['temporary'])))) }}">
            ...
            @endif
            @else
                href="{{ (asset(Arr::get($field, 'prefix', '').$field['value'])) }}">
            @endif
            {{ $file->original_name ?? $field['value'] }}
        </a>
